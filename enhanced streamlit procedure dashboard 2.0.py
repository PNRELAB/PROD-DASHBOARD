...
def local_to_sharepoint(local_path):
    ...
    if norm_local.startswith(norm_base_local):
        rel = norm_local[len(norm_base_local):].lstrip(r"\\/")  # relative path
...
with st.sidebar:
    ...
    with st.expander("✏️ Edit Procedure"):
        ...
        if procedures:
            proc_options = [p["text"] for p in procedures]
            ...
            if proc_to_edit:
                ...
                current_text = current_proc.get("text", "")
        ...
        if isinstance(link, dict) and link.get("type") == "file":
            ...
            if file_path and os.path.exists(file_path):
                with open(file_path, "rb") as f:
                    col2.download_button(f"⬇️ Download {file_name}", f, file_name=file_name)
                if share_url:
