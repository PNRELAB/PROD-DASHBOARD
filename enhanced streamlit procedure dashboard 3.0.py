...
with st.sidebar:
    ...
    if selected_test == "<No tests yet>": selected_test = None
    ...
    with st.expander("🗑️ Delete Test"):
        if selected_test and st.button(f"Delete '{selected_test}' Test", key="del_test"):
    ...
    with st.expander("✏️ Edit Procedure"):
        ...
        if procedures:
            ...
            if proc_to_edit:
                ...
                new_link = st.text_input("Update Link", value=current_link)
...
if procedures:
    for idx, proc in enumerate(procedures):
        ...
        if proc not in old_procs:
            ...
            col1.markdown(f"<div class='proc-row'>{text}</div>", unsafe_allow_html=True)
        if isinstance(link,dict) and link.get("type")=="file":
            ...
            if file_path and os.path.exists(file_path):
                with open(file_path,"rb") as f:
                    col2.download_button(f"⬇️ {file_name}", f, file_name=file_name)
                if share_url: col2.markdown(f"[🔗 Open in SharePoint]({share_url})", unsafe_allow_html=True)
            else:
                if share_url: col2.markdown(f"[🔗 Open in SharePoint]({share_url})", unsafe_allow_html=True)
                else: col2.write("⚠️ File missing")
