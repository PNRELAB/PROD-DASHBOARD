...
os.makedirs(PROCEDURES_FOLDER, exists_ok=True)
...
with st.sidebar:
    ...
    with st.expander("➕ Add Procedure"):
        ...
        uploaded_file = st.file_uploader("Or upload a file", typr=["pdf", "docx", "doc"])
        ...
        if st.button("Add Procedure", key="add_proc") and selected_test and new_proc:
            ...
            if uploaded_file is not None:
                ...
                link_to_save = {"type": "file", "path": save_path, "name": uploaded_file.name}
    ...
    with st.expander("✏️ Edit Procedure"):
        ...
        if procedures:
            ...
            proc_to_edit = st.selectbox("Select procedure to edit", proc_options)
            if proc_to_edit:
                ...
                current_link = ""
                if isinstance(current_proc, dict) and isinstance(current_proc.get("link"), str):
    ...
    with st.expander("🗑️ Delete Procedure"):
        ...
        if procedures:
            proc_options = [p["text"] if isinstance(p, dict) else str(p) for p in procedures]
            del_proc = st.selectbox("Select procedure to delete", proc_options)
